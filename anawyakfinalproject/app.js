...
function rHome(el) {
  ...
  '<div class="card-rose" style="padding:22px;margin-bottom:20px;animation:fadeUp .5s ease">' +
    ...
    '</div>' +
    '<div style="text-align:center;font-size:11px;color:var(--text-soft)">' + (isAr?'كيف حالك اليوم؟':'How are you feeling today?') + '</div>' +
  '</div>' +
...
async function doSignUp() {
  ...
  var hash = await hashPw(pass);
  var newProfile = { n1:n1, n2:n2, fam:fam, code:genCode() };
...
function openTonight() {
  ...
  sh.querySelector('.sheet').innerHTML =
    ...
    '<div style="text-align:center;padding:8px 0">' +
      ...
      '<div style="font-family:\'Cormorant Garamond\',serif;color:var(--rose);font-size:20px;font-weight:700;margin-bottom:6px">' + (isAr?'فكرة الليلة!':'Tonight\'s Idea!') + '</div>' +
      ...
      '<div style="display:flex;gap:10px;margin-top:20px">' +
        ...
        '<button class="btn-rose" style="padding:12px;font-size:14px" onclick="closeSheet(\'tn-sh\');T(isAr?\'استمتعا بليلتكما! 💕\':\'Enjoy your night! 💕\')">' + (isAr?'ممتاز! 💕':'Let\'s do it! 💕') + '</button>' +
      '</div>' +
    '</div>';
